...
// LiveCounters holds the cached total up/down (kept warm by the collector and
// merged into /api/state and the WebSocket stream).
...
func StartCollector(ctx context.Context, cfg *config.Config, pool *pgxpool.Pool,
	...
	// that die with the process are not lost
	sup.PersistUsage = func() {
