...
func (s *Server) lookupCustom(p string) *httputil.ReverseProxy {
        ...
        for path, route := range s.customs {
                // Match on segment boundaries ONLY: a raw prefix match would let a
                // 3-char custom path like "/ap" swallow every /api/* request and take
                // the whole panel (API included) offline.
                if (p == path || strings.HasPrefix(p, path+"/")) && len(path) > bestLen {
