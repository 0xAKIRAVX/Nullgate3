...
func (b *Builder) Build(ctx context.Context) (*BuiltResult, error) {
        ...
        if reality.Priv != "" && global["vless-reality"] {
                // Transport is switchable (REALITY_NET): "tcp" (default, vision flow) or
                // "grpc" — gRPC is the only way to serve Reality-gRPC on Railway, because
                // the edge downgrades origin traffic to HTTP/1.1 (gRPC needs h2) and only
                // one TCP Proxy exists; raw bytes through it + Xray doing h2 itself work.
                rnet := b.Cfg.RealityNet
                if rnet != "grpc" {
                        rnet = "tcp"
                }
                ...
                // Vision is TCP-only — gRPC accounts must NOT carry a flow.
                ...
                if rnet == "grpc" {
                        stream["grpcSettings"] = map[string]any{"serviceName": b.Cfg.RealitySvc}
                }
