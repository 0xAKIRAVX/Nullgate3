...
func EnsurePanelSettings(ctx context.Context, pool *pgxpool.Pool, sni string, sessionHours int) error {
        ...
        // seed only when the row is genuinely absent: treating a transient DB
        // error as "no settings yet" would silently reset the panel to defaults
...
func EnsureReality(ctx context.Context, pool *pgxpool.Pool, gen func() (map[string]any, error)) error {
        ...
        // only regenerate when the keys are really absent — a transient DB error
        // must never mint a new keypair (that would invalidate every issued
...
func LoadPanelSettings(ctx context.Context, pool *pgxpool.Pool, sni string, sessionHours int) (map[string]any, error) {
        ...
        if err := json.Unmarshal(raw, &m); err != nil {
                return nil, fmt.Errorf("parse panel settings: %w", err)
        }
        return m, nil
}
